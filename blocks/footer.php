<footer class="footer">
	<div class="container">
		<div class="row align-items-center">
			<div class="col-md-4 col-sm-12">
				<h4>Psigma Corp. Internacional</h4>
				<div class="row">
					<div class="col">
						<ul>
							<li>EE. UU.</li>
							<li>Colombia</li>
							<li>Brasil</li>
							<li>Ecuador</li>
							<li>Perú</li>
							<li>Guatemala</li>
						</ul>
					</div>
					<div class="col">
						<ul>
							<li>Honduras</li>
							<li>Costa Rica</li>
							<li>Panamá</li>
							<li>Venezuela</li>
							<li>Nicaragua</li>
							<li>El Salvador</li>
						</ul>
					</div>
				</div>
			</div>
			<div class="col-md-4 col-sm-12"> 
				<h5>Síguenos</h5>
				<ul class="social">
					<li>
						<a href="#" class="fb"><i class="fa fa-facebook fa-2x" aria-hidden="true"></i></a>
					</li>
					<li>
						<a href="#" class="tw"><i class="fa fa-twitter fa-2x" aria-hidden="true"></i></a>
					</li>
					<li>
						<a href="#" class="in"><i class="fa fa-linkedin fa-2x" aria-hidden="true"></i></a>
					</li>
					<li>
						<a href="#" class="gp"><i class="fa fa-google-plus fa-2x" aria-hidden="true"></i></a>
					</li>
					<li>
						<a href="#" class="yt"><i class="fa fa-youtube-play fa-2x" aria-hidden="true"></i></a>
					</li>
				</ul>
			</div>
			<div class="col-md-4 col-sm-12">
				<button class="mainBtn" id="burguer-button4">
					Quiero más información
				</button>
			</div>
			<div class="w-100"></div>
			<div class="col">
				<p class="text-center copyright">Copyright © 2017 Psigma Corp Pruebas Psicotécnicas competencias laborales DISC +571 6230800</p>
			</div>
		</div>
	</div>
</footer>